...
/***
 * <p>
 * A Class for parsing price data for each a Flat Files with time series data for stocks. This class
 * follows the Polygon.io Flat Files data format, with the following header:
 * </p>
 * 
 * <p>
 * ticker (str) volume (long) open (double) close (double) high (double) low (double) window_start
 ...
 * </p>
 * <p>
 * The class expects the Data to be well formatted. This means each stock must have the exact same
 * amount of DataPoints. Beyond that make sure that elements are already sorted in the way the
 * Strategy should be tested on them.
 * </p>
 * 
 * <p>
...
public class CSVData implements Data {
    ...
    /***
     * HashMap containing all the parsed financial data. Each asset receives its assigned List of
     * Data Records.
     */
    ...
    /***
     * Initialization of the Data Class by parsing the CSV File. Then set up of the values
     * classvariable.
     * 
    ...
    public Data init(String fileString) throws Exception {
        ...
        // The amount of prices available is given by the amount of lines without the
        // header.
    ...
    private HashMap<String, List<DataPoint>> parseFile(String pathString) throws Exception {
        ...
        while (allLinesIterator.hasNext()) {
            ...
            try {
                ...
                parsedData.get(ticker)
                        .add(new PriceDataPoint(ticker, (Integer) Integer.parseInt(line[1]),
                ...
                // The ticker is new and we create a List with the first encountered DataPoint
                // in it.
                DataPoint dataPoint = new PriceDataPoint(ticker,
                        (Integer) Integer.parseInt(line[1]), Double.parseDouble(line[2]),
                        Double.parseDouble(line[3]), Double.parseDouble(line[4]),
                        Double.parseDouble(line[5]), new Date(Long.parseLong(line[6])),
                        (Integer) Integer.parseInt(line[7]));
    ...
    /***
     * <p>
     * Returns the ID of the {@code CSVData} Object.
     * </p>
     * 
     * <p>
     * This is meant to allow for search in e.g. a List, so an optional way to e.g. identify a
     * {@code CSVData} Object in a list. If none is provided during initialization, a default Id
     * will be supplied.
     * </p>
     */
    @Override
    ...
    /***
     * Returns the {@code size} of the Objects which is defined as the amount of lines, excluding
     * the header.
     */
    @Override
    public Integer size() {
    ...
    /***
     * <p>
     * Returns the {@code values } of the CSVData Object.
     * </p>
     * 
     * <p>
     * {@code values } is a HashMap containing all the parsed financial data. Each asset receives
     * its assigned List of Data Records.
     * </p>
     */
    @Override
    public HashMap<String, List<DataPoint>> getValues() throws Exception {
        if (values.isEmpty()) {
            throw new Exception(
                    "No Data was assigned CSVData Object. Make sure to call init() before trying to access values.");
        } else {
            return values;
        }
    }
